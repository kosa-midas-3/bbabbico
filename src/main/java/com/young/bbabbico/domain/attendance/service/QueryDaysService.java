...
public class QueryDaysService {
    ...
    public AttendanceListResponse execute(String name, short month) {
        return AttendanceListResponse.builder()
                .days(createDays(name, month))
                .build();
    }

    private List<Long> createDays(String name, short month) {
        User user = userFacade.getUserByName(name);
        List<Attendance> attendanceList = attendanceRepository.findByUserAndDate_Month(user, month);
